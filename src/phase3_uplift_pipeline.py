...
def parse_args() -> argparse.Namespace:
    ...
    develop = subparsers.add_parser("develop", help="Fit with train and validation only.")
    develop.add_argument("--outcome", choices=["visit", "conversion", "both"], default="both")
    develop.add_argument("--mode", choices=["smoke", "full"], default="smoke")
    develop.add_argument("--freeze", action="store_true")
    score = subparsers.add_parser("score-test", help="Score frozen models on test features.")
    score.add_argument("--outcome", choices=["visit", "conversion", "both"], default="both")
    run_all = subparsers.add_parser("all", help="Run full development, freeze, and score test.")
    run_all.add_argument("--outcome", choices=["visit", "conversion", "both"], default="both")
...
def check_model_matrix(feature_names: list[str], outcome: str) -> None:
    forbidden = {
        "exposure", "source_row_id", "data_partition", "split", "split_stratum",
        "crossfit_fold", "visit" if outcome == "conversion" else "conversion",
    }
...
def construct_x_pseudo_outcome(
    ...
    """Construct the single X-Learner target D exactly as taught."""
    treated = treatment == 1
    control = ~treated
...
def train_models(
    ...
    s_model = fit_binary_model(
        lgb, s_train, train.outcome, s_validation_factual, validation.outcome,
        binary_params, feature_names + ["treatment"],
    )
    ...
    s_mu0 = s_model.predict(s_validation_0, num_iteration=prediction_iterations(s_model)).astype(np.float32)
    s_mu1 = s_model.predict(s_validation_1, num_iteration=prediction_iterations(s_model)).astype(np.float32)
    ...
    t_mu0_model = fit_binary_model(
        lgb, train.features[control_train], train.outcome[control_train],
        validation.features[control_validation], validation.outcome[control_validation],
        binary_params, feature_names,
    )
    t_mu1_model = fit_binary_model(
        lgb, train.features[treated_train], train.outcome[treated_train],
        validation.features[treated_validation], validation.outcome[treated_validation],
        binary_params, feature_names,
    )
    t_mu0 = t_mu0_model.predict(validation.features, num_iteration=prediction_iterations(t_mu0_model)).astype(np.float32)
    t_mu1 = t_mu1_model.predict(validation.features, num_iteration=prediction_iterations(t_mu1_model)).astype(np.float32)

    # X-Learner as taught: create one D dataset and train one g(X).
    train_mu0 = t_mu0_model.predict(train.features, num_iteration=prediction_iterations(t_mu0_model)).astype(np.float32)
    train_mu1 = t_mu1_model.predict(train.features, num_iteration=prediction_iterations(t_mu1_model)).astype(np.float32)
    ...
    gc.collect()
    return {
        "models": {"s_learner": s_model, "t_mu0": t_mu0_model, "t_mu1": t_mu1_model, "x_learner": x_model},
...
def constant_dictionary(value: str, length: int) -> pa.DictionaryArray:
    return pa.DictionaryArray.from_arrays(pa.array(np.zeros(length, dtype=np.int8)), pa.array([value]))
...
def write_prediction_artifact(
    ...
    observed = pa.array(data.outcome, type=pa.uint8()) if data.outcome is not None else pa.nulls(length, type=pa.uint8())
    table = pa.table({
...
def save_development(
    config: dict[str, Any], outcome: str, mode: str, validation: PartitionData,
    ...
    write_json(model_directory / "metadata.json", {
    ...
    for model_name in LEARNERS:
        artifact_hash = canonical_hash({"run_config_hash": model_config_hash, "model_name": model_name})
        write_prediction_artifact(
            prediction_directory / f"{model_name}.parquet", validation, outcome,
            model_name, artifact_hash, source_sha, result["predictions"][model_name],
        )
    report_directory = PROJECT_ROOT / "reports" / "tables" / "phase3"
    report_directory.mkdir(parents=True, exist_ok=True)
    result["diagnostics"].to_csv(report_directory / f"{mode}_{outcome}_model_diagnostics.csv", index=False)
    return model_directory, model_config_hash


def freeze_specification(outcome: str, model_directory: Path, model_config_hash: str, manifest: dict[str, Any]) -> None:
    write_json(model_directory / "FROZEN.json", {
        "outcome": outcome,
        "model_config_hash": model_config_hash,
        "source_file_sha256": manifest["raw_file_sha256"],
        "test_outcomes_used": False,
...
def run_development(config: dict[str, Any], outcomes: list[str], mode: str, freeze: bool) -> None:
    ...
    if not phase3_file.exists() or not manifest_path.exists():
        raise FileNotFoundError("Run src/prepare_phase3_data.py before model development.")
    ...
    for outcome in outcomes:
        ...
        validation = load_partition(phase3_file, config["features"], "validation", outcome, fraction)
        result = train_models(config, outcome, train, validation)
        model_directory, model_config_hash = save_development(config, outcome, mode, validation, result, manifest)
        if freeze:
            freeze_specification(outcome, model_directory, model_config_hash, manifest)
        print(f"{outcome}: {mode} complete in {result['runtime_seconds'] / 60:.1f} minutes; config {model_config_hash[:12]}")
...
def run_test_scoring(config: dict[str, Any], outcomes: list[str]) -> None:
    ...
    for outcome in outcomes:
        ...
        if not frozen_path.exists():
            raise FileNotFoundError(f"No frozen specification for {outcome}. Run full development with --freeze.")
        ...
        clear_generated_directory(prediction_directory, (".parquet",))
        for model_name in LEARNERS:
            artifact_hash = canonical_hash({"run_config_hash": metadata["model_config_hash"], "model_name": model_name})
            write_prediction_artifact(
                prediction_directory / f"{model_name}.parquet", test, outcome,
                model_name, artifact_hash, source_sha, predictions[model_name],
            )
...
def main() -> None:
    ...
    if args.stage == "develop":
        run_development(config, outcomes, args.mode, args.freeze)
    elif args.stage == "score-test":
        run_test_scoring(config, outcomes)
    elif args.stage == "all":
        run_development(config, outcomes, "full", freeze=True)
        run_test_scoring(config, outcomes)
    else:
